...
export default function CharacterCard({ character }: CharacterCardProps) {
    ...
    return (
        <div key={character.id} className=" bg-white rounded-lg flex flex-col items-center md:flex-row md:justify-around shadow-2xl p-5 w-3/4 lg:w-3/5 m-auto mt-5">
            <Link to={`/character/${character.id}`}>
                <button className=" p-4">
                    <img src={character.images[0]} alt="Imagen personaje" className="rounded-lg" onClick={() => console.log(character.id)}/>
                </button>
            </Link>
